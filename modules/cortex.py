...
SECRETS_REGEX = {"AWS Access Key": re.compile(r'(AKIA[0-9A-Z]{16})'), "Stripe Standard Key": re.compile(r'(sk_live_[0-9a-zA-Z]{24})'), "Google API Key": re.compile(r'(AIza[0-9A-Za-z-_]{35})'), "Slack Token": re.compile(r'(xox[baprs]-[0-9a-zA-Z]{10,48})'), "RSA Private Key": re.compile(r'(-----BEGIN RSA PRIVATE KEY-----)')}
...
class TaintTracker:
    ...
    def walk(self, node):
        ...
        if node.type == "Literal" and isinstance(node.value, str):
            ...
            if 16 <= len(val) <= 128 and " " not in val and "," not in val and not val.startswith(('/', 'http')):
                if not UUID_REGEX.match(val) and not HEX_HASH_REGEX.match(val):
                    if is_whitelisted(val): continue
        ...
        for key, val in vars(node).items():
            ...
            elif hasattr(val, "type"): self.walk(val)
