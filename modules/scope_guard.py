...
SAAS_BLACKLIST = [
    ...
    "vanta.com", "github.io", "github.com"
]
            ...
            if cname:
                for saas in SAAS_BLACKLIST:
                    if saas in cname:
                        ...
                        is_safe = False
                        break
            if is_safe: safe_targets.append(domain)

    session.subdomains = safe_targets

    # Robust SQLite Zombie Purge (Schema Agnostic)
    if out_of_scope:
        try:
            cursor = session.conn.cursor()
            # Attempt to delete by searching the raw JSON data string to bypass unknown schemas
            for domain, _, _ in out_of_scope:
                cursor.execute("DELETE FROM subdomains WHERE data LIKE ?", (f"%{domain}%",))
                cursor.execute("DELETE FROM live_hosts WHERE data LIKE ?", (f"%{domain}%",))
            session.conn.commit()
        except Exception:
            pass
...
def sanitize_state_graph(session, config):
    ...
    safe_hosts, purged_count, domain_cache = [], 0, {}
    ...
    for host in live_hosts:
        ...
        if domain not in domain_cache:
            ...
            is_safe = True
            if any(saas in domain for saas in SAAS_BLACKLIST): is_safe = False
            elif cname and any(saas in cname for saas in SAAS_BLACKLIST): is_safe = False
